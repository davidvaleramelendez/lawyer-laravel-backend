    ...
    public function get_all(Request $request) {
        try {
            ...
            if(Helper::get_user_permissions(6)==1) {
                $letters  =Letters::with('cases', 'user')->where("isErledigt","0")->where('frist_date', '>', date('Y-m-d', strtotime("-60 days")))->where( 'frist_date', '<', Carbon::now());
                $casedocs = Casedocs::where("isErledigt","0")->where('frist_date', '>', date('Y-m-d', strtotime("-60 days")))->where( 'frist_date', '<', Carbon::now())->orderBy('frist_date', 'ASC')->get();
                
                $invoices = invoice::with('user', 'created_user', 'case')->whereNot("status","paid")->where('invoice_due_date', '>', date('Y-m-d', strtotime("-60 days")))->where( 'invoice_due_date', '<', Carbon::now());
